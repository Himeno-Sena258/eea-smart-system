...
@Tag(name = "系统管理员模块", description = "包含账号开户、组织架构、批量导入与安全审计全套后台功能")
@RequireRoles("ADMIN")
public class AdminController {
    ...
    @Operation(summary = "【账号管理】管理员控制台首页")
    ...
    @GetMapping("/users")
    @Operation(summary = "【账号管理】分页查询用户列表", description = "支持按关键字、角色代码、组织结构过滤")
    ...
    @GetMapping("/users/{id}")
    @Operation(summary = "【账号管理】查询用户明细信息")
    ...
    @Operation(summary = "【账号管理】修改账号启用 / 禁用状态")
    ...
    @Operation(summary = "【账号管理】重置用户密码", description = "如果不传 newPassword，则默认重置为 123456")
