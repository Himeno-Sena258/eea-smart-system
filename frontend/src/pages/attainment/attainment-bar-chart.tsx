...
export function AttainmentBarChart({
  ...
  const groupWidth = items.length > 0 ? plotWidth / items.length : plotWidth
  ...
  return (
    <div>
      ...
      {items.length === 0 ? (
        <div className="grid min-h-[240px] place-items-center rounded-lg border border-dashed border-slate-200 text-sm font-bold text-slate-400">
          暂无课程目标达成度数据
        </div>
      ) : (
